...
export interface SearchConfig {
  ...
  /**
   * Whether to use rows format in API responses
   * When true, the API will return data in the format { data: { rows: [...] } }
   * Default: true
   */
  useRows?: boolean;
}
...
export function useBridgeCrud<T extends EntityData, U = Partial<T>, V = Partial<T>>(
  ...
  // State
  const [searchTerm, setSearchTermInternal] = useState(searchConfig.initialSearchTerm || '');
  const [debouncedSearchTerm, setDebouncedSearchTerm] = useState(searchConfig.initialSearchTerm || '');
  const searchDebounceTimeoutRef = useRef<NodeJS.Timeout | null>(null);

  // Función para establecer el término de búsqueda con debounce
  const setSearchTerm = useCallback((term: string) => {
    ...
    // Establecer un nuevo timeout para actualizar el término de búsqueda debounced
  ...
  const searchParams = {
    ...
    orderBy,
    orderDir,
    q: debouncedSearchTerm, // Usar el término de búsqueda con debounce
    filters,
    // Tomar useRows de searchConfig si está definido, de lo contrario usar true como valor predeterminado
    useRows: searchConfig.useRows !== undefined ? searchConfig.useRows : true,
    // Añadir entity para raw fetch
    entity: entityConfig.endpoint
  };

  // Use query hook for fetching data with raw fetch
  const {
    data: items,
    meta,
    isLoading: loading,
    ...
    refetch: refresh,
    isOffline
  } = useBridgeQuery(service, 'search', searchParams, undefined, true); // Use raw fetch by default
  ...
  return {
    ...
    // Search
    searchTerm, // Devolver el valor visual (no debounced) para la UI
    setSearchTerm,
    filters,
    addFilter,
    removeFilter,
